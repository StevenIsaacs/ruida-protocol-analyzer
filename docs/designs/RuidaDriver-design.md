...
### The RdsAdapter Class -- rpascript/rds_adapter.py
...
#### User Interface
The `RdsAdapter` creates a Terminal User Interface (TUI) using [Textualize/textual](https://github.com/textualize/textual/) This is used to display status and reply data as well as support user interaction.
...
##### Main TUI Window
This is for display of log messages generated by `RdsAdapter` and any of the underlying components. It automatically scrolls and supports using the mouse scroll wheel to scroll to previous messages. This will support a maximum of 1000 lines.
##### Command Lines
...
#### Threading
The `RdsAdapter` will use `asyncio`  for cooperative threading. Thus the differences between `asyncio` and the underlying layers use of `threading` need to be accounted for.
#### Listening
The `RdsAdapter` registers with the `RdDriver` as both status and reply listeners. Data received this way is converted into a text form for display in the TUI Status Side Panel.
## Ruida Driver (L6)
...
+ rpascript
	| rds_adapter.py     # The RdsAdapter class for scripted sessions.
```
## Iterations
...
Currently, `rpa-script` supports only Ruida controller commands. In order to test L6 (RdDriver) and implement L7 (RdsAdapter) new scripting commands need to be added to support starting and stopping sessions. These commands should be lower case to make them visually distinctive in a text form and should include:
...
This work is possible because of the hard work of others.
 ...
 - [Ruida Protocol Analyzer](https://github.com/StevenIsaacs/ruida-protocol-analyzer)
