...
The generated document sequence is: PRD, Implementation Plan, and Task List.
...
Include a completion and approval check box for each task in the Task List. A phase cannot be marked as complete until all tasks in the phase are marked as complete.

Include instructions in the PRD for generating the corresponding Implementation Plan once the PRD has been approved. Similarly, include instructions in the Implementation Plan for generating a Task List which can be delegated to subagents. Implementation can not begin until the Implementation Plan has been approved and the corresponding Task List has been approved by the user.
...
Visualization must be done with python and the charts must be rendered on a web page. Include a layout of the web page for displaying the plots and including pull downs for changing views, styles, colors, and other plot parameters. Automatically start a server and open the browser to display the plots in a single window with multiple tabs.
...
## Special Considerations
  ...
  - Context Menus (Right-Click): Using custom JavaScript handling move bokeh panning control to the middle mouse button so that the previously described right-click context menus can be implemented.
...
- The base file name is bokeh_plotting_<DOC_TYPE>.md where <DOC_TYPE> is replaced with PRD for Product Requirements Document, IMP for Implementation Plan, and TSK for Task List.

## Additional Implementation Requirements

- Create the .venv-bokeh virtual environment in the project root directory and install all dependencies in that virtual environment. All invocations of python will first activate this environment.